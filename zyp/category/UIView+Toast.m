...
static const NSString * CSToastAddCallbackKey   = @"CSToastAddCallbackKey";
...
- (void)addToastCallback:(void(^)(void))callback {
    void (^callbackOld)(void) = objc_getAssociatedObject(self, &CSToastTapCallbackKey);
    if (callbackOld == NULL) {
        objc_setAssociatedObject(self, &CSToastTapCallbackKey, callback, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(id)position title:(NSString *)title image:(UIImage *)image {
    ...
    // 一秒5个字
    double dura = (NSTimeInterval)message.length/5;
    duration = MAX(dura, duration);
      ...
      tapCallback:(void(^)(void))tapCallback
{
    ...
    objc_setAssociatedObject (self, &CSToastTapCallbackKey, tapCallback, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)hideToast:(UIView *)toast {
    objc_setAssociatedObject (toast, &CSToastTapCallbackKey, NULL, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - CSToastVerticalPadding/*稍微弄高点*/);
}
