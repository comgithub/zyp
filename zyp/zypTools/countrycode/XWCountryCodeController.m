...
- (void)creatSubviews{
    ...
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, Z_STATUS_BAR_HEIGHT, Z_SCREEN_WIDTH, Z_SCREEN_HEIGHT-Z_STATUS_BAR_HEIGHT-Z_BOTTOM_BAR_HEIGHT) style:UITableViewStylePlain];
    ...
    NSString *language =[[NSUserDefaults standardUserDefaults] objectForKey:Z_LanguageKey];
    ...
    [_sortedNameDict.allValues enumerateObjectsUsingBlock:^(NSArray * obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj enumerateObjectsUsingBlock:^(NSString * obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj containsString:inputText]) {
                ...
                if (![strongSelf resultsContain:obj]) {
...
- (BOOL)resultsContain:(NSString *)obj {
